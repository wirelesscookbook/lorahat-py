...
def transmitFunction(message: list, timeout: int) -> int :
    ...
    global intSet
    if not intSet :
        #irq_device = InputDevice(irqPin, pull_up=None, active_state=True)
        #irq_device.when_activated = checkTransmitDone
        irq_device = Button(irqPin)
        irq_device.when_pressed = checkTransmitDone
        intSet = True
    # Set rxen and txen pin state for transmitting packet
    if txenPin != -1 and rxenPin != -1 :
        txen_device = OutputDevice(txenPin)
        rxen_device = OutputDevice(rxenPin)
        txen_device.on()
        rxen_device.off()
    ...
    if txenPin != -1 :
        txen_device = OutputDevice(txenPin)
        txen_device.off()  # This sets the pin to LOW
